...
// Upgraded input field to fit glassmorphism theme with pointer cursor
...
export function SignupForm() {
  ...
  return (
    <>
      {/* Pure CSS Animation for the continuous shine effect on the sign-up button */}
      ...
      {/* Main Container Wrapper - Text switched to bright variants */}
      <main className="mx-auto flex w-full max-w-md flex-col gap-6 px-6 py-16 text-white">
        ...
        <form className="grid gap-4 rounded-3xl border border-white/20 bg-white/5 p-6 shadow-xl backdrop-blur-xl" onSubmit={onSubmit}>
          ...
          <label className="grid gap-1">
            ...
            {/* Relative wrapper for custom eye button positioning */}
            <div className="relative w-full">
              <input
                ...
                required
                type={showPassword ? "text" : "password"} // Dynamically switches visibility
                value={password}
              />
              
              {/* Eye View/Hide Action Button */}
              <button
                type="button"
          ...
          {/* Sign up Button with continuous shine effect */}
          <button
        ...
        <p className="text-sm font-medium text-white/70">
          Already registered?{" "}
          <Link className="font-bold text-sky-400 hover:text-sky-300 transition-colors cursor-pointer" href="/login">
            Sign in
          </Link>
        </p>
      </main>
